...
public class Main {
	...
	private long packet_num=0;//设置输入的抓包数目
	
	public JButton btnStart;
	public Thread getPacketThread;	//抓包线程
	public JButton btnStop;
	public JTextField getPacketNum;//设置抓取数量
	...
	/**
	 * 这里主要是界面Swing编程，不多写注释了。
	 */
	private void initialize() {
		frame = new JFrame();
		frame.setTitle("\u8ba1\u7b97\u673a\u7f51\u7edc\u8bfe\u8bbe");	//unicode~
		...
		btnStart.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int pack_num = 10;
				try {
					pack_num = Integer.parseInt(getPacketNum.getText());
				} catch (Exception e2) {
					// TODO: handle exception
					System.out.println("输入框不为数值");
				}
				getPacketThread = new MyJpcap(pack_num,selectedDevice, main);
		...
		getPacketNum = new JTextField();
		getPacketNum.setBounds(531, 0, 150, 23);
		getPacketNum.setText("输入抓包数，不输默认10");
		getPacketNum.addFocusListener(new FocusListener() {
		    ...
		    public void focusGained(FocusEvent e) {//获得焦点时
		        ...
		        getPacketNum.setText("");
		    }
		});
		frame.getContentPane().add(getPacketNum);
